...
export const handleAuthCommand = async (interaction: ChatInputCommandInteraction) => {
    ...
    if (subcommandGroup === 'blacklist') {
        if (subcommand === 'add') {
            ...
            const reason =
                ...
                i18n.t('runtime.commands.auth.blacklist.defaultReason', {}, interaction.locale);
            const now = Math.floor(Date.now() / 1000);

            try {
                await dbRun(
                    'INSERT OR REPLACE INTO Blacklist (user_id, guild_id, reason, added_at, added_by) VALUES (?, ?, ?, ?, ?)',
                    [user.id, guildId, reason, now, interaction.user.id],
                );
                await interaction.reply({
                    content: i18n.t('runtime.commands.auth.blacklist.added', { user: user.tag }, interaction.locale),
                    flags: MessageFlags.Ephemeral,
                });
            } catch (error) {
                logger.error(error);
                await interaction.reply({
                    content: i18n.t('runtime.commands.auth.blacklist.error.add', {}, interaction.locale),
                    flags: MessageFlags.Ephemeral,
                });
            }
            ...
            try {
                ...
                await interaction.reply({
                    content: i18n.t('runtime.commands.auth.blacklist.removed', { user: user.tag }, interaction.locale),
                    flags: MessageFlags.Ephemeral,
                });
            } catch (error) {
                logger.error(error);
                await interaction.reply({
                    content: i18n.t('runtime.commands.auth.blacklist.error.remove', {}, interaction.locale),
                    flags: MessageFlags.Ephemeral,
                });
            }
            ...
            try {
                ...
                if (list.length === 0) {
                    await interaction.reply({
                        content: i18n.t('runtime.commands.auth.blacklist.empty', {}, interaction.locale),
                        flags: MessageFlags.Ephemeral,
                    });
                    return;
                }
                ...
                logger.error(error);
                await interaction.reply({
                    content: i18n.t('runtime.commands.auth.blacklist.error.list', {}, interaction.locale),
                    flags: MessageFlags.Ephemeral,
                });
            }
        }
    ...
    if (subcommand === 'help') {
        const embed = new EmbedBuilder()
            .setTitle(i18n.t('runtime.commands.auth.help.title', {}, interaction.locale))
            .setColor(0x5865f2)
            .addFields(
                {
                    name: i18n.t('runtime.commands.auth.help.setupGuide', {}, interaction.locale),
                    value: i18n.t('runtime.commands.auth.help.setupSteps', {}, interaction.locale),
                    inline: false,
                },
                {
                    name: i18n.t('runtime.commands.auth.help.commandList', {}, interaction.locale),
                    value: i18n.t('runtime.commands.auth.help.commands', {}, interaction.locale),
                    inline: false,
                },
            );
    ...
    if (subcommand === 'setup') {
        ...
        try {
            ...
            if (existingConfig) {
                await dbRun('UPDATE GuildConfig SET notification_channel_id = ? WHERE guild_id = ?', [
                    channel.id,
                    guildId,
                ]);
            } else {
                ...
                await dbRun(
                    'INSERT INTO GuildConfig (guild_id, notification_channel_id, auth_panel_message) VALUES (?, ?, ?)',
                    [guildId, channel.id, defaultMessage],
                );
            }

            await interaction.reply({
                content: i18n.t(
                    'runtime.commands.auth.channel.success',
            ...
            logger.error(error);
            await interaction.reply({
                content: i18n.t('runtime.commands.auth.channel.error', {}, interaction.locale),
                flags: MessageFlags.Ephemeral,
            });
        }
        ...
        try {
            ...
            if (!config) {
                await interaction.reply({
                    content: i18n.t('runtime.commands.auth.config.notFound', {}, interaction.locale),
                    flags: MessageFlags.Ephemeral,
                });
                return;
            }
            ...
            const questionList =
                questions.length > 0
                    ? questions.map((q, i) => `${i + 1}. ${stripOptionalMarker(q)}`).join('\n')
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    {
                        name: i18n.t('runtime.commands.auth.config.channel', {}, interaction.locale),
                        value: config.notification_channel_id
                            ? `<#${config.notification_channel_id}>`
                            : i18n.t('runtime.common.notSet', {}, interaction.locale),
                    },
                    {
                        name: i18n.t('runtime.commands.auth.config.panel', {}, interaction.locale),
                        value: config.panel_channel_id
                            ? `<#${config.panel_channel_id}>`
                            : i18n.t('runtime.common.notSet', {}, interaction.locale),
                    },
                    {
                        name: i18n.t('runtime.commands.auth.config.questions', {}, interaction.locale),
                        value: questionList,
                    },
                )
                .setColor(0x00ff00);
            ...
            logger.error(error);
            await interaction.reply({
                content: i18n.t('runtime.commands.auth.config.error', {}, interaction.locale),
                flags: MessageFlags.Ephemeral,
            });
        }
        ...
        const user = interaction.options.getUser('user', true);
        try {
            const history = await dbAll<AuthApplication>(
                'SELECT * FROM AuthApplications WHERE user_id = ? AND guild_id = ? ORDER BY applied_at DESC LIMIT 10',
                [user.id, guildId],
            );

            if (history.length === 0) {
                await interaction.reply({
                    content: i18n.t('runtime.commands.auth.history.empty', {}, interaction.locale),
                    flags: MessageFlags.Ephemeral,
                });
                return;
            }
            ...
            logger.error(error);
            await interaction.reply({
                content: i18n.t('runtime.commands.auth.history.error', {}, interaction.locale),
                flags: MessageFlags.Ephemeral,
            });
        }
        ...
        try {
            ...
            if (existingConfig) {
                await dbRun('UPDATE GuildConfig SET dm_notification_enabled = ? WHERE guild_id = ?', [
                    dm_notification_value,
                    guildId,
                ]);
            } else {
                const defaultMessage = i18n.t('runtime.commands.auth.setup.placeholder', {}, interaction.locale);
                await dbRun(
                    'INSERT INTO GuildConfig (guild_id, dm_notification_enabled, auth_panel_message) VALUES (?, ?, ?)',
                    [guildId, dm_notification_value, defaultMessage],
                );
            }
            const status = enable
                ? i18n.t('runtime.commands.auth.dm_notification.status.enabled', {}, interaction.locale)
                : i18n.t('runtime.commands.auth.dm_notification.status.disabled', {}, interaction.locale);
            await interaction.reply({
                content: i18n.t('runtime.commands.auth.dm_notification.success', { status }, interaction.locale),
                flags: MessageFlags.Ephemeral,
            });
        } catch (error) {
            logger.error(error);
            await interaction.reply({
                content: i18n.t('runtime.commands.auth.dm_notification.error', {}, interaction.locale),
                flags: MessageFlags.Ephemeral,
            });
        }
        ...
        try {
            const config = await dbGet<GuildConfig>('SELECT * FROM GuildConfig WHERE guild_id = ?', [guildId]);
            const stats = await dbAll<{ status: string; count: number }>(
                'SELECT status, COUNT(*) as count FROM AuthApplications WHERE guild_id = ? GROUP BY status',
                [guildId],
            );
            ...
            logger.error(error);
            await interaction.reply({
                content: i18n.t('runtime.commands.auth.status.error', {}, interaction.locale),
                flags: MessageFlags.Ephemeral,
            });
        }
    }
