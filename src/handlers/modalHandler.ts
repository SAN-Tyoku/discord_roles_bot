    ...
    if (customId === 'auth_panel_setup_modal') {
        ...
        try {
            ...
            if (!config || !config.notification_channel_id) {
                await interaction.reply({
                    content: i18n.t('runtime.handlers.modal.panelSetup.noChannel', {}, interaction.locale),
            ...
            if (config.panel_channel_id && config.panel_message_id) {
                try {
                    ...
                    logger.warn(
                        i18n.t(
                            'runtime.handlers.modal.panelSetup.oldPanelDeleteError',
                            {},
            ...
            if (!channel) {
                await interaction.reply({
                    content: i18n.t('runtime.handlers.modal.panelSetup.notTextChannel', {}, interaction.locale),
                    flags: MessageFlags.Ephemeral,
                });
                return;
            }
            ...
            const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
                new ButtonBuilder()
                    ...
                    .setLabel(
                        i18n.t(
                            'runtime.handlers.modal.panelSetup.buttonLabel',
                            {},
                            interaction.guildLocale || undefined,
                        ),
                    )
                ...
                new ButtonBuilder()
                    ...
                    .setLabel(
                        i18n.t(
                            'runtime.handlers.modal.panelSetup.cancelLabel',
                            {},
                            interaction.guildLocale || undefined,
                        ),
                    )
            ...
            if (config) {
                await dbRun(
                    'UPDATE GuildConfig SET panel_channel_id = ?, panel_message_id = ?, auth_panel_message = ? WHERE guild_id = ?',
                    [channel.id, sentMessage.id, message, guildId],
                );
            } else {
                await dbRun(
                    'INSERT INTO GuildConfig (guild_id, notification_channel_id, panel_channel_id, panel_message_id, auth_panel_message) VALUES (?, ?, ?, ?, ?)',
                    [guildId, null, channel.id, sentMessage.id, message],
                );
            }

            await interaction.reply({
                content: i18n.t('runtime.handlers.modal.panelSetup.success', {}, interaction.locale),
                flags: MessageFlags.Ephemeral,
            });
        } catch (error) {
            logger.error(error);
            ...
            if (err.code === 50013) {
                await interaction.reply({
                    content: i18n.t('runtime.handlers.modal.panelSetup.permissionError', {}, interaction.locale),
                    flags: MessageFlags.Ephemeral,
                });
            } else {
                await interaction.reply({
                    content: i18n.t('runtime.handlers.modal.panelSetup.error', {}, interaction.locale),
                    flags: MessageFlags.Ephemeral,
                });
            }
        }
        ...
        try {
            ...
            if (config) {
                ...
                await dbRun(
                    'INSERT INTO GuildConfig (guild_id, modal_questions, auth_panel_message) VALUES (?, ?, ?)',
                    [guildId, questionsJson, defaultMessage],
                );
            }

            await interaction.reply({
                content: i18n.t(
                    'runtime.handlers.modal.questionConfig.success',
            ...
            logger.error(error);
            await interaction.reply({
                content: i18n.t('runtime.handlers.modal.questionConfig.error', {}, interaction.locale),
                flags: MessageFlags.Ephemeral,
            });
        }
        ...
        try {
            ...
            if (existing) {
                await interaction.editReply(
                    i18n.t('runtime.handlers.modal.application.alreadyPending', {}, interaction.locale),
                );
                return;
            }
            ...
            if (!config || !config.notification_channel_id) {
                await interaction.editReply(
                    i18n.t('runtime.handlers.modal.application.noChannelError', {}, interaction.locale),
                );
                return;
            }
            ...
            const userHistory = await dbGet<{ count: number }>(
                'SELECT COUNT(*) as count FROM AuthApplications WHERE user_id = ? AND guild_id = ?',
                [interaction.user.id, guildId],
            );

            if (userHistory && userHistory.count >= historyLimit) {
                await dbRun(
                    `DELETE FROM AuthApplications WHERE id IN (
            ...
            await dbRun(
                `INSERT INTO AuthApplications (user_id, guild_id, status, answers, applied_at) VALUES (?, ?, 'pending', ?, ?) `,
            ...
            if (notificationChannel) {
                ...
                const embed = new EmbedBuilder()
                    .setTitle(
                        i18n.t('runtime.handlers.modal.application.title', {}, interaction.guildLocale || undefined),
                    )
                    .setAuthor({ name: interaction.user.tag, iconURL: interaction.user.displayAvatarURL() })
                    .addFields(
                        {
                            name: i18n.t(
                                'runtime.handlers.modal.application.userId',
                                {},
                                interaction.guildLocale || undefined,
                            ),
                        ...
                        {
                            name: i18n.t(
                                'runtime.handlers.modal.application.createdAt',
                                {},
                                interaction.guildLocale || undefined,
                            ),
                ...
                const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
                    new ButtonBuilder()
                        ...
                        .setLabel(
                            i18n.t(
                                'runtime.handlers.modal.application.approve',
                                {},
                                interaction.guildLocale || undefined,
                            ),
                        )
                    ...
                    new ButtonBuilder()
                        ...
                        .setLabel(
                            i18n.t(
                                'runtime.handlers.modal.application.reject',
                                {},
                                interaction.guildLocale || undefined,
                            ),
                        )
                ...
                const sentMessage = await notificationChannel.send({
                    content: i18n.t(
                        'runtime.handlers.modal.application.notificationContent',
                ...
                await dbRun(
                    'UPDATE AuthApplications SET notification_message_id = ? WHERE user_id = ? AND guild_id = ? AND status = "pending"',
        ...
        try {
            ...
            const now = Math.floor(Date.now() / 1000);
            await dbRun(
                'UPDATE AuthApplications SET status = ?, processed_at = ?, processor_id = ?, notes = ? WHERE user_id = ? AND guild_id = ? AND status = "pending"',
            ...
            if (interaction.message && interaction.message.embeds.length > 0) {
                ...
                if (oldEmbed) {
                    const newEmbed = EmbedBuilder.from(oldEmbed)
                        ...
                        .setTitle(
                            i18n.t(
                                'runtime.handlers.modal.application.rejectedTitle',
                                {},
                                interaction.guildLocale || undefined,
                            ),
                        )
                        .addFields(
                            {
                                name: i18n.t(
                                    'runtime.handlers.modal.application.rejectReason',
                                    {},
                                    interaction.guildLocale || undefined,
                                ),
                                value: reason,
                            ...
                            {
                                name: i18n.t(
                                    'runtime.handlers.modal.application.processor',
                                    {},
                                    interaction.guildLocale || undefined,
                                ),
