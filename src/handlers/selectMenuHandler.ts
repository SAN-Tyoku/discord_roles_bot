import { RoleSelectMenuInteraction, EmbedBuilder, TextChannel, Role } from 'discord.js';
...
export const handleSelectMenu = async (interaction: RoleSelectMenuInteraction) => {
    ...
    if (customId.startsWith('auth_role_select_')) {
        ...
        await interaction.update({
            content: i18n.t('runtime.handlers.selectMenu.processing', {}, interaction.locale),
            components: [],
        });
        ...
        if (!member) {
            await interaction.editReply(
                i18n.t('runtime.handlers.selectMenu.error.memberNotFound', {}, interaction.locale),
            );
            return;
        }
        ...
        if (!actor || !botMember) {
            await interaction.editReply(
                i18n.t('runtime.handlers.selectMenu.error.userInfoFetchFailed', {}, interaction.locale),
            );
            return;
        }
        ...
        for (const item of selectedRoles.values()) {
            ...
            if (item instanceof Role) {
                ...
                if (!fetched) {
                    await interaction.editReply(
                        i18n.t(
                            'runtime.handlers.selectMenu.error.roleIdNotFound',
                            { roleId: item.id },
                            interaction.locale,
                        ),
                    );
            ...
            if (role.comparePositionTo(botHighestRole) >= 0) {
                await interaction.editReply(
                    i18n.t(
                        'runtime.handlers.selectMenu.error.roleHierarchyBot',
                        { role: role.toString() },
                        interaction.locale,
                    ),
                );
            ...
            if (!isOwner && role.comparePositionTo(actorHighestRole) >= 0) {
                await interaction.editReply(
                    i18n.t(
                        'runtime.handlers.selectMenu.error.roleHierarchyUser',
                        { role: role.toString(), actorRole: actorHighestRole.toString() },
                        interaction.locale,
                    ),
                );
                return;
        ...
        try {
            ...
            await dbRun(
                'UPDATE AuthApplications SET status = ?, processed_at = ?, processor_id = ? WHERE user_id = ? AND guild_id = ? AND status = "pending"',
            ...
            if (config && config.notification_channel_id) {
                const channel = (await interaction.guild?.channels
                    .fetch(config.notification_channel_id)
                    ...
                    if (message && message.embeds.length > 0) {
                        ...
                        if (oldEmbed) {
                            ...
                            const newEmbed = EmbedBuilder.from(oldEmbed)
                                ...
                                .setTitle(
                                    i18n.t(
                                        'runtime.handlers.selectMenu.approved.embedTitle',
                                        {},
                                ...
                                .addFields(
                                    {
                                        name: i18n.t(
                                            'runtime.handlers.selectMenu.approved.grantedRoles',
                                            {},
                                            interaction.guildLocale || undefined,
                                        ),
                                        value: roleMentions,
                                    ...
                                    {
                                        name: i18n.t(
                                            'runtime.handlers.selectMenu.approved.approver',
                                            {},
                                            interaction.guildLocale || undefined,
                                        ),
                    ...
                    await member.send({
                        embeds: [
                            new EmbedBuilder()
                                .setTitle(
                                    i18n.t(
                                        'runtime.handlers.selectMenu.dm.title',
                                        {},
                                        interaction.guildLocale || undefined,
                                    ),
                                )
                                .setDescription(
                                    i18n.t(
                                        'runtime.handlers.selectMenu.dm.description',
                                        {
                                            guild:
                                                interaction.guild?.name ||
                                                i18n.t('runtime.common.server', {}, interaction.locale),
                                        },
                                        interaction.guildLocale || undefined,
                                    ),
                                )
                                .addFields({
                                    ...
                                    value: selectedRoles.map((r) => r.name).join(', '),
                                    inline: false,
                                })
                                .setColor(0x00ff00)
                                .setTimestamp(),
            ...
            await interaction.editReply(
                i18n.t('runtime.handlers.selectMenu.success', { count: selectedRoles.size }, interaction.locale),
            );
        } catch (error) {
            logger.error(error);
